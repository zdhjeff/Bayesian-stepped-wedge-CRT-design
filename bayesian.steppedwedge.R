...
bayes.pwr = function(xxx, fam, p, t=6, Nij=NULL, N=20, tx.eff, mu0, m=0, sigma.e=1,
  ...
  if (fam=="binomial" | fam=="poisson"){
    ...
    out = c(mean(trt.effect), R.h0, exp(l), exp(u),
            mean(check.this[[1]]),r_hat,ess,trt.effect=list(trt.effect),trt.effect2=list(trt.effect2)) # check.this[[1]] is the variance of the random effect
    class(out)='trial'
  }
  if (fam=="gaussian"){
    ...
    out = c(mean(trt.effect), R.h0, l, u,
            mean(check.this[[1]]),r_hat,ess,trt.effect=list(trt.effect),trt.effect2=list(trt.effect2))
    class(out)='trial'
  }
